...
import (
	...
	router "local.com/leobrada/ztsfc_http_pep/router_no_tls"
	"local.com/leobrada/ztsfc_http_pep/sf_info"
	"log"
	"net/http"
	"net/url"
)
...
func loadServicePool2(config env.Config_t) (service_pool map[string]sf_info.ServiceFunctionInfo) {
...
func loadSfPool2(config env.Config_t) (sf_pool map[string]sf_info.ServiceFunctionInfo) {
...
func main() {
	...
	service_pool := loadServicePool2(env.Config)

	// Load Service Function Pool from configuration file
	sf_pool := loadSfPool2(env.Config)
