...
func NewRouter(_service_pool map[string]sf_info.ServiceFunctionInfo, _sf_pool map[string]sf_info.ServiceFunctionInfo) (*Router, error) {
    ...
    // Frontend Loggers
    router.logger = log.New(LogWriter{}, "", log.LstdFlags)
...
func (router *Router) logConnections(req *http.Request) {
    router.logger.Printf(",%s,%s,%s,%t,%t,%s,success\n", req.RemoteAddr,req.TLS.ServerName, matchTLSConst(req.TLS.Version),
        req.TLS.HandshakeComplete, req.TLS.DidResume, matchTLSConst(req.TLS.CipherSuite))
}

func (router *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
    // fmt.Printf("Serving Request for %s\n", req.TLS.ServerName)
    router.logConnections(req)
